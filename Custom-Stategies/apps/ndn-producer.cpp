...
Producer::OnInterest(shared_ptr<const Interest> interest)
{
  ...
  // NS_LOG_DEBUG(Simulator::Now().ToDouble(Time::S) << "\t" << "Interest Received" << "\t" << Nname << "\t" << interest->getName().toUri() << "\t" << seq);
  ...
  // data->setPitSize(tempPit.size());

  // NS_LOG_DEBUG(Simulator::Now().ToDouble(Time::S) << "\t" << "Data Sent" << "\t" << Nname << "\t" << node->GetId() << "\t" << data->getName().toUri() << "\t" << seq);
