...
Consumer::SendPacket()
{
  ...
  // const nfd::Pit& tempPit = node->GetObject<ndn::L3Protocol>()->getForwarder()->getPit();
  // interest->setPitSize(tempPit.size());
  ...
  // double cur = Simulator::Now().ToDouble(Time::S);
  // m_cal.insert(std::make_pair(seq,cur));
  // NS_LOG_DEBUG(cur << "\t" << "Interest Sent" << "\t" << Nname << "\t" << node->GetId() << "\t" << interest->getName().toUri() << "\t" << seq);
...
Consumer::OnData(shared_ptr<const Data> data)
{
  ...
  // std::string Nname = ns3::Names::FindName(node);
  // double cur = Simulator::Now().ToDouble(Time::S);
  // NS_LOG_DEBUG( cur - m_cal.find(seq)->second << "\t" << "App Delay" << "\t" << Nname << "\t" << node->GetId() << "\t" << data->getName().toUri() << "\t" << seq);
  // m_cal.erase(seq);
  // ns3::Ptr<ns3::Node> node = ns3::NodeList::GetNode(ns3::Simulator::GetContext());
  // std::string Nname = ns3::Names::FindName(node);
  // NS_LOG_DEBUG(Simulator::Now().ToDouble(Time::S) << "\t" << "Data Received" << "\t" << Nname << "\t" << data->getName().toUri() << "\t" << seq);
...
Consumer::OnNack(shared_ptr<const lp::Nack> nack)
{
  ...
  // ns3::Ptr<ns3::Node> node = ns3::NodeList::GetNode(ns3::Simulator::GetContext());
  // std::string Nname = ns3::Names::FindName(node);
  // NS_LOG_DEBUG(Simulator::Now().ToDouble(Time::S) << "\t" << "Nack Received" << "\t" << Nname << "\t" << nack->getInterest().getName().toUri() << "\t" << "Reason" << "\t" << nack->getReason());
