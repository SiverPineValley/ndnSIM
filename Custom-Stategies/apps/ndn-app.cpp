...
App::OnInterest(shared_ptr<const Interest> interest)
{
  ...
  // NS_LOG_DEBUG(Simulator::Now().ToDouble(Time::S) << "\t" << "Interest Received" << "\t" << Nname << "\t" << node->GetId() << "\t" << interest->getName().toUri() << "\t" << seq);
...
App::OnData(shared_ptr<const Data> data)
{
  ...
  // ns3::Ptr<ns3::Node> node = ns3::NodeList::GetNode(ns3::Simulator::GetContext());
  // std::string Nname = ns3::Names::FindName(node);
  // NS_LOG_DEBUG(Simulator::Now().ToDouble(Time::S) << "\t" << "Data Received" << "\t" << Nname << "\t" << node->GetId() << "\t" << data->getName().toUri() << "\t" << seq);
...
App::OnNack(shared_ptr<const lp::Nack> nack)
{
  ...
  // ns3::Ptr<ns3::Node> node = ns3::NodeList::GetNode(ns3::Simulator::GetContext());
  // std::string Nname = ns3::Names::FindName(node);
  // NS_LOG_DEBUG(Simulator::Now().ToDouble(Time::S) << "\t" << "Nack Received" << "\t" << Nname << "\t" << node->GetId() << "\t" << nack->getInterest().getName().toUri() << "\t" << "Reason" << "\t" << nack->getReason());
  // @TODO Implement
