...
Interest::wireEncode(EncodingImpl<TAG>& encoder) const
{
  ...
  // PitSize
  // uint32_t myTempInt = this->getPitSize();
  // totalLength += encoder.prependByteArray(reinterpret_cast<uint8_t*>(&myTempInt), sizeof(myTempInt));
  // totalLength += encoder.prependVarNumber(sizeof(myTempInt));
  // totalLength += encoder.prependVarNumber(tlv::PitSize);
...
Interest::decode02()
{
  ...
  // New Field
  // PitSize?
  // auto val = m_wire.find(tlv::PitSize);
  // uint32_t myPitSize = 0;
  // if (ele != m_wire.elements_end() && ele->type() == tlv::PitSize) {
  //   m_pitsize = readNonNegativeInteger(*ele);
  //   ++ele;
  // }
  // else {
  //   std::memcpy(&myPitSize, val->value(), sizeof(myPitSize));
  //   m_pitsize = myPitSize;
  // }
  // val = m_wire.find(tlv::PitSize);
...
Interest::decode03()
{
  ...
  for (const Block& ele : m_wire.elements()) {
    switch (ele.type()) {
      ...
      // case tlv::PitSize: {
      //   if (lastEle >= 9) {
      //     BOOST_THROW_EXCEPTION(Error("PitSize element is out of order"));
      //   }
      //   m_pitsize = readNonNegativeInteger(ele);
      //   lastEle = 9;
      //   break;
...
// New Field
// uint32_t
// Interest::getPitSize() const
// {
//   return m_pitsize;
// }

// Interest&
// Interest::setPitSize(uint32_t ps)
// {
//   m_pitsize = ps;
//   m_wire.reset();
//   return *this;
