...
{
  if(ns3::Simulator::Now().ToDouble(ns3::Time::S) > 0)
  {
    ns3::Ptr<ns3::Node> node = ns3::NodeList::GetNode(ns3::Simulator::GetContext());
    if( ((node-> GetId() >= 0) && (node-> GetId() <= 2)) || ( node-> GetId() == 8) )
    {
      std::string Nname = ns3::Names::FindName(node);
      // uint8_t buffer[packet->GetSize ()]; 
      // packet->CopyData (buffer, packet->GetSize ());
      // Ptr<RegularWifiMac> r = DynamicCast<RegularWifiMac>(*this);
      // NS_LOG_DEBUG(ns3::Simulator::Now().ToDouble(ns3::Time::S) << "\t" << "packet NotifyTx" <<  "\t" << Nname << "\t" << node-> GetId() << "\t" << "Mac queue size" << "\t" << r->GetDcaTxop2()->GetQueue()->GetNPackets() );
    } 
  }
  m_macTxTrace (packet);
}
...
WifiMac::NotifyTxDrop (Ptr<const Packet> packet)
{
  if(ns3::Simulator::Now().ToDouble(ns3::Time::S) > 0)
  {
    ...
    if( ((node-> GetId() >= 0) && (node-> GetId() <= 2)) || ( node-> GetId() == 8) )
    {
      ...
      // packet->CopyData (buffer, packet->GetSize ());
      // Ptr<RegularWifiMac> r = DynamicCast<RegularWifiMac>(*this);
      // NS_LOG_DEBUG(ns3::Simulator::Now().ToDouble(ns3::Time::S) << "\t" << "packet NotifyTxDrop" <<  "\t" << Nname << "\t" << node-> GetId() << "\t" << r->GetDcaTxop2()->GetQueue()->GetNPackets() );
    } 
  }
...
WifiMac::NotifyRx (Ptr<const Packet> packet)
{
  if(ns3::Simulator::Now().ToDouble(ns3::Time::S) > 0)
  {
    ...
    if( ((node-> GetId() >= 0) && (node-> GetId() <= 2)) || ( node-> GetId() == 8) )
    {
      ...
      // packet->CopyData (buffer, packet->GetSize ());
      // Ptr<RegularWifiMac> r = DynamicCast<RegularWifiMac>(*this);
      // NS_LOG_DEBUG(ns3::Simulator::Now().ToDouble(ns3::Time::S) << "\t" << "packet NotifyRx" <<  "\t" << Nname << "\t" << node-> GetId() << "\t" << r->GetDcaTxop2()->GetQueue()->GetNPackets() );
    } 
  }
  m_macRxTrace (packet);
}
...
WifiMac::NotifyPromiscRx (Ptr<const Packet> packet)
{
  if(ns3::Simulator::Now().ToDouble(ns3::Time::S) > 0)
  {
    ...
    if( ((node-> GetId() >= 0) && (node-> GetId() <= 2)) || ( node-> GetId() == 8) )
    {
      ...
      // packet->CopyData (buffer, packet->GetSize ()); 
      // Ptr<RegularWifiMac> r = DynamicCast<RegularWifiMac>(*this);
      // NS_LOG_DEBUG(ns3::Simulator::Now().ToDouble(ns3::Time::S) << "\t" << "packet NotifyPromiscRx" <<  "\t" << Nname << "\t" << node-> GetId() << "\t" << r->GetDcaTxop2()->GetQueue()->GetNPackets() );
    }
  }
...
WifiMac::NotifyRxDrop (Ptr<const Packet> packet)
{
  if(ns3::Simulator::Now().ToDouble(ns3::Time::S) > 0)
  {
    ...
    if( ((node-> GetId() >= 0) && (node-> GetId() <= 2)) || ( node-> GetId() == 8) )
    {
      ...
      // packet->CopyData (buffer, packet->GetSize ());
      // Ptr<RegularWifiMac> r = DynamicCast<RegularWifiMac>(*this);
      // NS_LOG_DEBUG(ns3::Simulator::Now().ToDouble(ns3::Time::S) << "\t" << "packet NotifyRxDrop" <<  "\t" << Nname << "\t" << node-> GetId() << "\t" << r->GetDcaTxop2()->GetQueue()->GetNPackets() );
    }     
  }
